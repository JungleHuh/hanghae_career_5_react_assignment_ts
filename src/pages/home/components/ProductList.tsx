...
*/
import { useProductsStore, selectProducts, selectHasNextPage, selectIsLoading, selectTotalCount } from '@/store/product/productsStore';
...
export const ProductList: React.FC<ProductListProps> = ({
  ...
  const { isOpen, openModal, closeModal } = useModal();
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [isIndexErrorModalOpen, setIsIndexErrorModalOpen] = useState<boolean>(false);
  const [indexLink, setIndexLink] = useState<string | null>(null);
